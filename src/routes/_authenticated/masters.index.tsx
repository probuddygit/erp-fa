...
import { MASTERS, MASTER_GROUPS, findMaster } from "@/lib/mdm/registry";
...
function MasterCard({ m }: { m: MasterDef }) {
  const Icon = m.icon;
  return (
    <Link to="/masters/$master" params={{ master: m.key }} className="group">
      <Card className="h-full border-border/60 transition-all hover:border-primary/40 hover:shadow-elevated">
        ...
        <CardContent className="flex flex-wrap items-center gap-1.5 pt-0">
          ...
          <Badge variant="outline" className="font-mono text-[10px]">{m.codePrefix}</Badge>
        </CardContent>
      </Card>
    </Link>
  );
...
function MastersIndex() {
  const grouped = MASTER_GROUPS.map((g) => ({
    ...g,
    defs: g.keys.map(findMaster).filter(Boolean) as MasterDef[],
  })).filter((g) => g.defs.length);

  const groupedKeys = new Set(MASTER_GROUPS.flatMap((g) => g.keys));
  const ungrouped = MASTERS.filter((m) => !groupedKeys.has(m.key));
    ...
    <div className="mx-auto w-full max-w-6xl space-y-8 p-4 sm:p-6">
      <header className="flex flex-col gap-2">
        ...
        <div className="flex flex-wrap gap-1.5 pt-1">
          ...
          <Badge variant="outline" className="text-[10px]">
            {MASTERS.reduce((n, m) => n + m.fields.length, 0)} governed fields
          </Badge>
        </div>
      </header>
      ...
      {grouped.map((g) => (
        <section key={g.label} className="space-y-3">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {g.defs.map((m) => <MasterCard key={m.key} m={m} />)}
          </div>
        </section>
      ))}
      ...
      {ungrouped.length > 0 && (
        <section className="space-y-3">
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {ungrouped.map((m) => <MasterCard key={m.key} m={m} />)}
          </div>
        </section>
      )}
    </div>
